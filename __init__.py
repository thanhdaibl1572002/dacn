...
def classification_data():
    ...
    classification_new = pd.read_csv('dacn/classification_new.csv')
    classification_target = 'revenue'
    scaler = StandardScaler()
    for key in classification_datasets:
        X = classification_datasets[key].drop(columns=[classification_target])
        y = classification_datasets[key][classification_target]
        X_scaled = scaler.fit_transform(X)
        classification_datasets[key] = pd.DataFrame(X_scaled, columns=X.columns)
        classification_datasets[key][classification_target] = y
    X_new_scaled = scaler.transform(classification_new)
    classification_new = pd.DataFrame(X_new_scaled, columns=classification_new.columns)
...
def classification_evaluates(my_model, sk_model):
    ...
    sk_times = []
    dataset_names = []
    model_name = ' '.join(re.findall(r'[A-Z][a-z]*', type(my_model).__name__))
...
def regression_data():
    ...
    for key in regression_datasets:
        ...
        y = regression_datasets[key][regression_target]
        X_scaled = scaler.fit_transform(X)
        regression_datasets[key] = pd.DataFrame(X_scaled, columns=X.columns)
        regression_datasets[key][regression_target] = y
    X_new_scaled = scaler.transform(regression_new)
    regression_new = pd.DataFrame(X_new_scaled, columns=regression_new.columns)
    return regression_datasets, regression_new, regression_target
